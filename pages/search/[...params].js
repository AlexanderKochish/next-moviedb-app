...
export const getServerSideProps = async (context) => {
  ...
  const data = await fetch(
    `${BASE_URL}search/movie?api_key=${API_KEY}&query=${params}&page=${page}`
  );
      ...
      <div className="container mx-auto">
                  ...
                  <div className="flex flex-col mb-1 w-40 lg:w-48 xl:w-48 ">
                    <div className="relative w-40 h-52 lg:w-48 lg:h-72 xl:w-48 xl:h-72">
                      <Image
                        ...
                        alt={mov.original_title || mov.title || "poster"}
                        fill
        ...
        <div className="text-white flex space-x-3 items-center py-5 justify-center">
          <Link
            ...
            href={`/search/${params}/?page=${page - 1}`}
          >
            <button
              className={
                currentPage <= 1
                  ? "hidden"
                  : "py-3 px-4 rounded-md bg-rose-600 m-2"
              }
            >
              <MdArrowBackIos />
            </button>
