...
const ModalMovRow = ({setOpen,open,movId}) => {
    const[movImageTrailer,setImageTrailer] = useState('')
   
    useEffect(()=>{
        async function getVideo(){
            const res = await fetch(`https://api.themoviedb.org/3/movie/${movId}/videos?api_key=${API_KEY}&language=en-US`)
            const data = await res.json()
            
            return setImageTrailer(data.results[0]?.key)
        }
        getVideo()
    },[movId])
  ...
  return (
    <div className='absolute z-30 top-0 left-0 w-full min-h-screen'>
        <div className='flex flex-col w-full h-screen items-center justify-center space-y-10'>
            ...
            <ReactPlayer 
                url={`https://www.youtube.com/watch?v=${movImageTrailer}`}
                width='100%'
                height='100%' 
                controls={true}
            />
